...
Implement `toaster` from Shadcn UI to handle all notifications across the application. Replace any existing custom toast notifications or alerts with the unified Shadcn Toaster component. Ensure toasts are styled according to the current active theme.
...
### R7. Build Integrity
After all changes, `npm run build`, `npm run lint`, and `npx tsc --noEmit` pass with zero errors. No `@ts-ignore` or `as any` workarounds.
...
## Follow-up — 2026-06-21T08:35:11Z
...
Address and systematically resolve all issues identified in the recent audit reports (`performance_report.md`, `code_health_report.md`, and `security_report.md`) for the InkDown project.

Working directory: ~/documents/antigravity/zealous-tesla
Integrity mode: development

## Requirements

### R1. Fix Offline PWA Architecture
Convert the reader routes from Server Components to Client/Hybrid Components, ensuring the application can fetch and display cached Markdown files from Dexie.js when the network drops. Remove any dead code related to caching.

### R2. Eliminate Performance Bottlenecks
Resolve the scroll thrashing in the `ProgressBar` and `AutoScroller`, and move the synchronous, main-thread blocking Regex markdown parsing into Web Workers or chunked asynchronous processes. Lazy load heavy Streamdown dependencies using `next/dynamic`.

### R3. Implement Database Sync and Tenant Isolation
Implement the missing database synchronization API/Server Actions for highlights, bookmarks, and reading progress. Ensure every Drizzle ORM query strictly filters by `clerkId` to enforce tenant isolation. Add composite database indexes where necessary.

### R4. Resolve React Anti-Patterns
Fix the React ref mutation during the render phase in `MarkdownRenderer.tsx` and ensure all asynchronous local storage operations properly handle Promise rejections.

## Acceptance Criteria

### Technical Correctness
- [ ] The Next.js application builds successfully for production (`npm run build`).
- [ ] TypeScript compilation passes with zero errors (`npx tsc --noEmit`).
- [ ] ESLint passes with zero warnings or errors (`npm run lint`).

### Offline Verification
- [ ] The user can successfully navigate to a previously cached markdown file while the browser's network is set to 'Offline' and read it without encountering the fallback `~offline` page.
